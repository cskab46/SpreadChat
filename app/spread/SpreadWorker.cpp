    ...
    while (running) {
        if (mailbox >= 0) {
            ...
            while (numMessages > 0) {
                ...
                int mismatch = 0;
                int groupNum;
                SP_receive(mailbox, &svcType, sender, 1, &groupNum, groups, &msgType, &mismatch, 0, nullptr);
                // O spread passa o tamanho do buffer necessário como o negativo do valor do endian mismatch
                QByteArray message(-mismatch, '\0');
                SP_receive(mailbox, &svcType, sender, 1, &groupNum, groups, &msgType, &mismatch, message.size(), message.data());
